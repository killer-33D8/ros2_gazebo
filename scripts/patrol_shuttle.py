...
需先启动仿真 + Nav2，再运行本脚本。
  python3 scripts/patrol_shuttle.py
  或（若已安装）：ros2 run 对应包 patrol_shuttle.py
"""
import rclpy
...
class PatrolShuttleNode(Node):
    ...
    def _result_cb(self, future):
        ...
        self.get_logger().info("Goal reached (or failed). Switching to next point.")
        self._schedule_next()
